...
(def example-feed {:tray-id     tray-id
                   :url         tray-url
                   :included    [project-id]})
...
(deftest get-projects
  ...
  (binding [subject/*fetch* (constantly (StringReader. "some-xml-response"))
    ...
    (testing "works when given a single feed"
      (is (= [example-project]
             (subject/get-projects {:feeds     [example-feed]
                                    :prognosis [project-prognosis]}))))
    ...
    (testing "works when given multiple feeds"
      (is (= [example-project example-project]
             (subject/get-projects {:feeds [example-feed example-feed]}))))

    (testing "returns an empty array when no projects are included"
      (is (= []
             (subject/get-projects {:feeds [(merge example-feed {:included []})]}))))

    (testing "includes everything if included is nil"
      (is (= [example-project]
             (subject/get-projects {:feeds [(merge example-feed {:included nil})]}))))
    ...
    (testing "sorting"
      ...
      (let [project-1 {:description "b"
            ...
            project-5 {:description "d"
                       ...
                       :project-id  "d"
                       :tray-id     tray-id}
            feeds [(merge example-feed {:included ["a", "b", "c", "d", "e"]})]]
      ...
      (testing "returns errors and projects if only one tray fails for any reason"
        (binding [subject/*fetch* (fn [{:keys [tray-id]}] (if (= "fail-id" tray-id)
          ...
          (is (= [example-project (expected-error "some-error" "fail-id")]
                 (subject/get-projects {:feeds [example-feed (merge example-feed {:tray-id "fail-id"
                                                                                  :url     tray-url})]}))))))))

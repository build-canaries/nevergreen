...
function maximumPossibleFontSize(
...
export function ideal(
  ...
  let fontSize = maximumPossibleFontSize(
    sentences,
    elementWidthPx,
    charWidthScale,
    paddingEm,
  ...
  while (fontSize > MIN_FONT_SIZE) {
    const numberOfLines = sentences.map((sentence: string) =>
      linesRequired(
        sentence,
        elementWidthPx,
        charWidthScale,
    ...
    const actualHeight =
      elementHeightPx - totalPaddingPixels(fontSize, paddingEm)
...
export function ScaleText({
  sentences,
  children,
}: ScaleTextProps): ReactElement {
  const elementRef = useRef<HTMLDivElement>(null)
  const { height: fontHeight, width: fontWidth } =
    useContext(FontMetricsContext)
  const [{ elementHeight, elementWidth }, setElementSize] = useState({
    elementWidth: 0,
    elementHeight: 0,
  })
  ...
  const onResize = useCallback((currentSize: Measurable) => {
    setElementSize((previousSize) => {
      ...
      const heightChanged =
        Math.abs(currentSize.height - previousSize.elementHeight) > 1
      const widthChanged =
        Math.abs(currentSize.width - previousSize.elementWidth) > 1
  ...
  useEffect(() => {
    return () => {
      // clear cache when unmounted, this is probably OK because if this has unmounted it's likely the sentences have
  ...
  return (
    <div className={styles.body} ref={elementRef}>
      <div style={idealFontStyle}>{children}</div>
    </div>
  )
}

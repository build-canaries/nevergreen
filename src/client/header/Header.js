    ...
    return (
      <header role='banner' className={headerClassNames}>
        ...
        <nav role='navigation' className={styles.siteMenu}>
          ...
          <button className={styles.siteMenuToggle} onClick={toggleMenu} aria-label={toggleLabel}>
          ...
          <ul className={menuClassNames}>
            {
              items.map((item) => {
                ...
                return (
                  <li key={item.id}>
                    <NavLink to={`/${item.id}`} className={styles.menuItem} activeClassName={styles.active} onClick={hideMenu}
